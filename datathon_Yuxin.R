...
### BY KIRSTEN ###
library("readxl")
RAW<-read_excel("C:/Users/kirst/Desktop/RAW.xlsx")
View(RAW)
...
nRAW %>% filter(situation_type=='pp' & event_type=='Shot') %>% group_by(player_name) %>% summarise(totalshot=n()) %>% mutate(p=totalshot/sum(totalshot)) %>% arrange(desc(totalshot))
#mean=8.4 (t & f)
nRAW %>% filter(situation_type=='pp' & event_type=='Shot') %>% group_by(player_name) %>% summarise(totalshot=n()) %>% filter(totalshot>8.4) %>% arrange(desc(totalshot))
